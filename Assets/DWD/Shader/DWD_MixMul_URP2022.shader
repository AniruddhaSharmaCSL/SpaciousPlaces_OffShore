...
Shader "DWD/Effects/Scrolling Mix"
{
   Properties
   {
...
#define _USINGTEXCOORD1 1
	...
	void Ext_SurfaceFunction0 (inout Surface o, ShaderData d)
	{
		...
		half one = GetMix(sampleOne, _FXMixOne);
		half two = GetMix(sampleTwo, _FXMixTwo);

		half4 sampleMul = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, d.texcoord1.xy);

		half mulMix = GetMix(sampleMul, _FXMixMul);
		half mask = saturate(pow(abs(mulMix  - _MulOffset), _MulPow) * _MulBoost);
