...
Shader "DWD/Skybox/Depth Skybox"
{
   Properties
   {
	...
	_DepthLOD("Depth LOD", float) = 0.0
	_DepthPow("Depth Pow", float) = 1.0
	_DepthBoost("Depth Boost", float) = 1.0
	_DepthScale("Depth Scale", float) = 1.0
	[Header(Depth Pole Masking)]
	_DepthPoleMask("Pole Masking", Range(0,1)) = 0
	_DepthPoleDot("Pole Dot", Range(0,1)) = 1
	_DepthPoleCutoff("Pole Cutoff", Range(0,1)) = 0.3
	_DepthPolePow("Pole Pow", float) = 0.5
	_DepthPoleBoost("Pole Boost", float) = 1.0
...
#define _USINGTEXCOORD2 1
	...
	{	
		...
		uv.y = 1.0 - uv.y;
		half offset = 1.0 - SAMPLE_TEXTURE2D_LOD(_DepthTex, sampler_DepthTex, uv, _DepthLOD).r;				
